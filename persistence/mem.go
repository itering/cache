...
func (c *MemoryStore) Add(ctx context.Context, key string, value interface{}, expire time.Duration) error {
	return c.Cache.SetWithTTL(key, value, expire)
}

func (c *MemoryStore) Replace(ctx context.Context, key string, data interface{}, expire time.Duration) error {
	return c.Cache.SetWithTTL(key, data, expire)
}

func (c *MemoryStore) Increment(ctx context.Context, key string, data uint64) (uint64, error) {
	return 0, nil
}

func (c *MemoryStore) Decrement(ctx context.Context, key string, data uint64) (uint64, error) {
	return 0, nil
}
