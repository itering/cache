...
func TestCacheDuration(t *testing.T) {
	memoryStore := newRedisStore(1 * time.Minute)
...
func TestConcurrentRequest(t *testing.T) {
	memoryStore := newRedisStore(1 * time.Minute)
	cacheURIMiddleware := CacheByRequestURI(memoryStore, 5*time.Second)
...
var newRedisStore = func(defaultExpiration time.Duration) persistence.CacheStore {
	var redisTestServer = "localhost:6379"
	...
	if err == nil {
		...
		redisCache := persistence.NewRedisCache(pool, defaultExpiration, "")
		return redisCache
	}
